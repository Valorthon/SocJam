...
The architecture is unchanged from the MVP seam: every platform interaction still goes through the `SocialPlatformAdapter` interface. The registry routes Facebook to either the mock or `RealFacebookAdapter` and Instagram to either the mock or `RealInstagramAdapter` based on the per-platform adapter env flag (or auto-detect from `META_APP_ID` + `META_APP_SECRET`), with no change to feature code. Real image publishing — Instagram **and** Facebook image posts — needs `BLOB_READ_WRITE_TOKEN` so media URLs are publicly fetchable by Meta's crawler (setting it switches **all** uploads to Blob storage; see the storage section below). Without it, IG publish rejects the media (`Instagram could not process this media`) and FB image posts fail identically, while text-only flows keep working on local-disk storage.
...
| `tests/registry.test.ts` | `FACEBOOK_ADAPTER=real` routes to `RealFacebookAdapter`; `"mock"` and unset both fall back to mock; auto-detect from `META_APP_ID`+`META_APP_SECRET`; `INSTAGRAM_ADAPTER=real` routes `getPlatformAdapter("INSTAGRAM")` to `RealInstagramAdapter`. |
| `tests/real-facebook-adapter.test.ts` | Successful publish → correct permalink (POST params in body, URL query empty); 401 / Graph code 190 → `authExpired: true`; 429 → retryable; video-only media rejected with no Graph call; raw Meta error body **never** leaks; network failure → retryable (no exception); `checkAuth` returns `{ active, account }`; `fetchAnalytics` throws `AnalyticsNotImplementedError`. Stubs `globalThis.fetch` — no real Graph calls. |
| `tests/refresh-tokens.test.ts` | `CRON_SECRET` guard (missing/empty/wrong/all correct, constant-time compare); happy refresh path; decrypt failure → `RECONNECT_REQUIRED`; Graph refresh failure → `RECONNECT_REQUIRED`; production query excludes non-Meta platforms; missing Meta config → 500-style error. Uses DI-friendly `createRefreshTokensRunner`. |
...
- **This is a global storage-tier switch, not an Instagram toggle.** Once `BLOB_READ_WRITE_TOKEN` is set, *every* upload — drafts, FB image posts, future non-IG flows — moves to public Blob storage, not just Instagram-bound media. Operators should expect FB image uploads to leave local disk too.
...
- **Facebook image posts also require public URLs.** Both single-image (`/photos?url=`) and carousel uploads pass an image URL that Meta's crawler fetches server-side. On localhost/staging without `BLOB_READ_WRITE_TOKEN`, real FB image publishing fails the same way real IG publishing does — the crawler cannot reach an auth-gated localhost URL. Text-only FB posts are unaffected.
...
- `publishPost` runs the two-step Instagram container flow:
  ...
  - Poll `GET /{containerId}?fields=status_code` until `FINISHED`/`ERROR` (or timeout — configurable via the adapter's `pollIntervalMs`/`pollMaxAttempts` deps, 10s default). Polling stays well inside serverless/cron function budgets (~10s on Vercel hobby); on timeout it returns `retryable:true` so the target lands `FAILED` and is re-attempted on retry rather than producing a partial post.
...
- **Per-platform dispatch.** `src/lib/platforms/registry.ts` resolves a platform's adapter at call time. For Facebook: `isFacebookRealEnabled()` → `realFacebookAdapter`, else mock. For LinkedIn: `isLinkedInRealEnabled()` → `linkedInAdapter`, else mock. For Instagram: `isInstagramRealEnabled()` → `realInstagramAdapter`, else mock. All other platforms stay mock. `MOCK_PLATFORMS` is **not replaced** by the per-platform flags — the actual contract is: per-platform `<PLATFORM>_ADAPTER` flags pick real-vs-mock for platforms that have a real adapter, while `MOCK_PLATFORMS=true` remains the coarse allow-mocks-at-all gate for every other platform (with the flag unset, an unconfigured platform throws instead of silently mocking).
...
- **Config detection.** `src/lib/platforms/config.ts` exports `isFacebookRealEnabled()` / `isInstagramRealEnabled()` mirroring `isLinkedInRealEnabled()`. Each reads `<PLATFORM>_ADAPTER` (or `NEXT_PUBLIC_<PLATFORM>_ADAPTER` for client-side); `"real"`/`"mock"` win over auto-detect; unset falls back to auto-detect from `META_APP_ID` + `META_APP_SECRET`. **Auto-detect is a side effect to be aware of in shared environments:** setting `META_APP_ID`/`META_APP_SECRET` (e.g. for OAuth-only use) flips both FB and IG adapters to real with no explicit opt-in. Production deployments should set `FACEBOOK_ADAPTER` / `INSTAGRAM_ADAPTER` explicitly to avoid surprises.
